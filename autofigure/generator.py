...
def update_config_from_sdk(sdk_config) -> None:
    ...
    provider = _normalize_provider(sdk_config.generation_provider)
    ...
    if provider == 'openrouter':
        ...
        # Generic OpenAI-compatible providers (OpenAI/Zhipu/DeepSeek/SiliconFlow...)
        resolved_base_url = sdk_config.generation_base_url or DEFAULT_BASE_URL_BY_PROVIDER.get(provider, 'https://api.openai.com/v1')
        resolved_model = sdk_config.generation_model or DEFAULT_MODEL_BY_PROVIDER.get(provider, 'gpt-4o-mini')

        CONFIG['OPENROUTER_API_KEY'] = sdk_config.generation_api_key
        CONFIG['OPENROUTER_BASE_URL'] = resolved_base_url
        CONFIG['OPENROUTER_MODEL'] = resolved_model

        CONFIG['BIANXIE_API_KEY'] = sdk_config.generation_api_key
        CONFIG['BIANXIE_BASE_URL'] = resolved_base_url
        CONFIG['BIANXIE_CHAT_MODEL'] = resolved_model

        if provider == 'openai':
            CONFIG['OPENAI_API_KEY'] = sdk_config.generation_api_key
            CONFIG['OPENAI_BASE_URL'] = resolved_base_url
            CONFIG['OPENAI_MODEL'] = resolved_model
        elif provider == 'zhipuai':
            CONFIG['ZHIPUAI_API_KEY'] = sdk_config.generation_api_key
            CONFIG['ZHIPUAI_BASE_URL'] = resolved_base_url
            CONFIG['ZHIPUAI_MODEL'] = resolved_model
        elif provider == 'deepseek':
            CONFIG['DEEPSEEK_API_KEY'] = sdk_config.generation_api_key
            CONFIG['DEEPSEEK_BASE_URL'] = resolved_base_url
            CONFIG['DEEPSEEK_MODEL'] = resolved_model
        elif provider == 'siliconflow':
            CONFIG['SILICONFLOW_API_KEY'] = sdk_config.generation_api_key
            CONFIG['SILICONFLOW_BASE_URL'] = resolved_base_url
            CONFIG['SILICONFLOW_MODEL'] = resolved_model
...
def call_unified_llm(contents: List[Any], provider: Optional[str] = None,
    ...
    actual_provider = _normalize_provider(provider or CONFIG.get('LLM_PROVIDER', 'bianxie'))
    ...
    if actual_provider == 'gemini':
        ...
        if actual_base_url and not actual_base_url.endswith('/openai/') and not actual_base_url.endswith('/openai'):
            if actual_base_url.endswith('/'):
                ...
                actual_base_url = actual_base_url + '/openai/'
    elif actual_provider in OPENAI_COMPATIBLE_PROVIDERS:
        # All OpenAI-compatible providers share the same request shape.
        # Use provider-specific defaults first, then fall back to OpenRouter/BianXie keys for legacy compatibility.
        provider_key = actual_provider.upper()
        actual_base_url = (
            base_url
            or CONFIG.get(f'{provider_key}_BASE_URL')
            or CONFIG.get('OPENROUTER_BASE_URL')
            or CONFIG.get('BIANXIE_BASE_URL')
            or DEFAULT_BASE_URL_BY_PROVIDER.get(actual_provider)
        )
        actual_api_key = (
            api_key
            or CONFIG.get(f'{provider_key}_API_KEY')
            or CONFIG.get('OPENROUTER_API_KEY')
            or CONFIG.get('BIANXIE_API_KEY')
        )
        actual_model = (
            model
            or CONFIG.get(f'{provider_key}_MODEL')
            or CONFIG.get('OPENROUTER_MODEL')
            or CONFIG.get('BIANXIE_CHAT_MODEL')
            or DEFAULT_MODEL_BY_PROVIDER.get(actual_provider)
        )
    else:  # Unknown provider: fallback to bianxie-compatible routing
