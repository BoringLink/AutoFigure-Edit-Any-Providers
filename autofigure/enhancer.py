...
Now analyze this {code_format.upper()} code and create the comprehensive text-to-image prompt following the exact format above. Focus especially on converting every programmatic element into a specific, detailed visual description that perfectly matches the "{art_style}" artistic style while maintaining visual clarity and professional quality."""
        ...
        provider = (provider or "bianxie").strip().lower()
        ...
        if provider == "gemini" and base_url:
            ...
            if provider == "openrouter":
                ...
                actual_base_url = "https://open.bigmodel.cn/api/paas/v4"
            elif provider == "deepseek":
                actual_base_url = "https://api.deepseek.com/v1"
            elif provider == "siliconflow":
                actual_base_url = "https://api.siliconflow.cn/v1"
            else:
...
class ImageEnhancer:
    ...
    def enhance(
        ...
        provider = (self.config.enhancement_provider or "bianxie").strip().lower()
        if provider in ("zai", "zhipu"):
            provider = "zhipuai"
        ...
        elif provider in ("openai", "zhipuai", "deepseek", "siliconflow"):
            return self._enhance_with_openai_compatible(
                str(input_path), enhancement_input, output_path,
                style=art_style, input_type=input_type,
                api_key=api_key, base_url=base_url, model=model,
...
Transform the provided layout into this enhanced visualization now."""
    ...
    def _enhance_with_openai_compatible(
        self,
        input_path: str,
        enhancement_input: str,
        output_path: str,
        ...
        """
        Generic OpenAI-compatible image enhancement path for OpenAI/Zhipu/DeepSeek/SiliconFlow.
        """
        try:
            if not api_key:
                print(f"[{provider_name}] ERROR: API key not provided!")
                return None
            ...
            print(f"[{provider_name}] API: {actual_base_url}")
            print(f"[{provider_name}] Model: {model}")
            ...
            response = requests.post(actual_base_url, headers=headers, json=data, timeout=300)
            if response.status_code != 200:
                print(f"[{provider_name}] API error: {response.status_code} - {response.text[:500]}")
                return None

            result = response.json()
            return self._extract_image_from_openai_response(result, output_path, input_path, provider_name)
        except Exception as e:
            print(f"[{provider_name}] Enhancement failed: {e}")
            import traceback
            traceback.print_exc()
            return None
