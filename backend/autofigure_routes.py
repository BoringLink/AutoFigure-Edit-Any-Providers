...
from .error_utils import create_error_response, create_validation_error, create_not_found_error, create_internal_error, handle_exception
...
def _normalize_provider_name(provider: Optional[str], fallback: str = 'bianxie') -> str:
    """Normalize provider names and aliases to a canonical lowercase key."""
...
def _default_base_url(provider: str) -> str:
    return DEFAULT_BASE_URL_BY_PROVIDER.get(provider, DEFAULT_BASE_URL_BY_PROVIDER['bianxie'])


def _extract_text_from_openai_content(content: Any) -> Optional[str]:
    """
...
def _extract_from_messages_list(messages: Any) -> Optional[str]:
    ...
    for msg in reversed(messages):
        ...
        if role and role != 'user':
            continue
        extracted = _extract_text_from_openai_content(msg.get('content'))
        if extracted and extracted.strip():
            return extracted
    return None


def _extract_input_content(payload: Dict[str, Any], config: Optional[Dict[str, Any]] = None) -> str:
    """
    Normalize text input from multiple payload shapes.

    Compatible fields:
    - input_content / input / content / prompt
    - messages (OpenAI-style), where we read user message text
    """
    if not isinstance(payload, dict):
        return ''

    # Canonical direct keys first
    for key in ('input_content', 'input', 'content', 'prompt'):
        value = payload.get(key)
        if isinstance(value, str) and value.strip():
            return value
        extracted = _extract_text_from_openai_content(value)
        if extracted and extracted.strip():
            return extracted
        # Responses-style "input" may be an array of message objects.
        extracted_messages = _extract_from_messages_list(value)
        if extracted_messages and extracted_messages.strip():
            return extracted_messages

    # OpenAI-style messages
    extracted_messages = _extract_from_messages_list(payload.get('messages'))
    if extracted_messages and extracted_messages.strip():
        return extracted_messages

    # Config fallback (legacy compatibility)
...
def _build_session_config(data: Dict[str, Any]) -> Dict[str, Any]:
    """Build canonical session config from both nested config and top-level OpenAI-like payloads."""
    raw_config = data.get('config', {})
    config = raw_config if isinstance(raw_config, dict) else {}

    llm_provider = _normalize_provider_name(
        config.get('llmProvider') or config.get('llm_provider') or data.get('provider') or data.get('llm_provider'),
        fallback='zhipuai',
    )
    base_url = (
        config.get('baseUrl')
        ...
        or data.get('baseUrl')
        or _default_base_url(llm_provider)
    )
    model = (
        config.get('model')
        or data.get('model')
        or DEFAULT_MODEL_BY_PROVIDER.get(llm_provider, '')
    )
    api_key = (
        config.get('apiKey')
    ...
    methodology_provider = _normalize_provider_name(
        config.get('methodologyLlmProvider') or config.get('methodology_llm_provider') or llm_provider,
        fallback=llm_provider,
    )
    methodology_base_url = (
        config.get('methodologyLlmBaseUrl')
        or config.get('methodology_llm_base_url')
        or _default_base_url(methodology_provider)
    )
    methodology_model = (
        config.get('methodologyLlmModel')
        or config.get('methodology_llm_model')
        or model
        or DEFAULT_MODEL_BY_PROVIDER.get(methodology_provider, 'gemini-3.1-pro-preview')
    )
    methodology_api_key = (
        config.get('methodologyLlmApiKey')
        or config.get('methodology_llm_api_key')
        or api_key
        or ''
    )

    return {
        'content_type': config.get('contentType', config.get('content_type', 'paper')),
        'max_iterations': config.get('maxIterations', config.get('max_iterations', 5)),
        'quality_threshold': config.get('qualityThreshold', config.get('quality_threshold', 9.0)),
        'min_improvement': config.get('minImprovement', config.get('min_improvement', 0.2)),
        'human_in_loop': config.get('humanInLoop', config.get('human_in_loop', True)),
        'llm_provider': llm_provider,
        'api_key': api_key,
        'base_url': base_url,
        'model': model,
        'svg_width': config.get('svgWidth', config.get('svg_width', 1333)),
        'svg_height': config.get('svgHeight', config.get('svg_height', 750)),
        'enable_methodology_extraction': config.get('enableMethodologyExtraction', config.get('enable_methodology_extraction', True)),
        'methodology_llm_provider': methodology_provider,
        'methodology_llm_api_key': methodology_api_key,
        'methodology_llm_base_url': methodology_base_url,
        'methodology_llm_model': methodology_model,
        'enhancement_mode': config.get('enhancementMode', config.get('enhancement_mode', 'text2image_prompt')),
        'art_style': config.get('artStyle', config.get('art_style', '')),
        'enhancement_count': config.get('enhancementCount', config.get('enhancement_count', 3)),
        'enhancement_provider': _normalize_provider_name(
            config.get('enhancementProvider') or config.get('enhancement_provider') or llm_provider,
            fallback='bianxie',
        ),
    }
...
def extract_methodology(markdown_content: str, config: Dict[str, Any]) -> Optional[str]:
    ...
    try:
        # Mock mode for CI: return deterministic extracted text without external API call.
        if os.environ.get('AUTOFIGURE_MOCK') == '1':
            return markdown_content[:2000] if markdown_content else markdown_content
...
--- PAPER MARKDOWN END ---
        ...
        provider = _normalize_provider_name(provider, fallback='bianxie')
        if not base_url:
...
def require_autofigure():
    ...
    if not AUTOFIGURE_AVAILABLE:
        return create_internal_error('AutoFigure module not available. Please ensure svg_figure_generator.py is properly installed.', details={'autofigure_path': str(autofigure_path)})
    return None
...
def reset_autofigure_config():
    ...
    AUTOFIGURE_CONFIG['OPENAI_API_KEY'] = ''
    AUTOFIGURE_CONFIG['ZHIPUAI_API_KEY'] = ''
    AUTOFIGURE_CONFIG['DEEPSEEK_API_KEY'] = ''
    AUTOFIGURE_CONFIG['SILICONFLOW_API_KEY'] = ''
    ...
    AUTOFIGURE_CONFIG['OPENAI_BASE_URL'] = 'https://api.openai.com/v1'
    AUTOFIGURE_CONFIG['OPENAI_MODEL'] = ''
    AUTOFIGURE_CONFIG['ZHIPUAI_BASE_URL'] = 'https://open.bigmodel.cn/api/paas/v4'
    AUTOFIGURE_CONFIG['ZHIPUAI_MODEL'] = 'glm-4.7-flash'
    AUTOFIGURE_CONFIG['DEEPSEEK_BASE_URL'] = 'https://api.deepseek.com/v1'
    AUTOFIGURE_CONFIG['DEEPSEEK_MODEL'] = ''
    AUTOFIGURE_CONFIG['SILICONFLOW_BASE_URL'] = 'https://api.siliconflow.cn/v1'
    AUTOFIGURE_CONFIG['SILICONFLOW_MODEL'] = ''
...
def _get_effective_model(provider: str, model: str) -> str:
    if model:
        return model
    return DEFAULT_MODEL_BY_PROVIDER.get(provider, '')


def _apply_provider_runtime_config(config: Dict[str, Any]) -> None:
    """
    Apply per-request provider credentials into AUTOFIGURE_CONFIG.

    The underlying generator path is OpenAI-compatible, so all OpenAI-like providers
    are also mirrored into OPENROUTER/BIANXIE keys for backward compatibility.
    """
    provider = _normalize_provider_name(config.get('llm_provider'), fallback='bianxie')
    api_key = config.get('api_key', '')
    base_url = (config.get('base_url') or '').strip() or _default_base_url(provider)
    model = _get_effective_model(provider, (config.get('model') or '').strip())

    config['llm_provider'] = provider
    config['base_url'] = base_url
    config['model'] = model

    AUTOFIGURE_CONFIG['LLM_PROVIDER'] = provider

    if provider == 'gemini':
        AUTOFIGURE_CONFIG['GOOGLE_API_KEY'] = api_key
        AUTOFIGURE_CONFIG['GEMINI_BASE_URL'] = base_url
        if model:
            AUTOFIGURE_CONFIG['GEMINI_MODEL'] = model
        return

    # OpenAI-compatible providers (including OpenRouter and Zhipu)
    if _is_openai_compatible_provider(provider):
        # Legacy routing compatibility: many code paths still consult these keys.
        AUTOFIGURE_CONFIG['OPENROUTER_API_KEY'] = api_key
        AUTOFIGURE_CONFIG['OPENROUTER_BASE_URL'] = base_url
        if model:
            AUTOFIGURE_CONFIG['OPENROUTER_MODEL'] = model

        AUTOFIGURE_CONFIG['BIANXIE_API_KEY'] = api_key
        AUTOFIGURE_CONFIG['BIANXIE_BASE_URL'] = base_url
        if model:
            AUTOFIGURE_CONFIG['BIANXIE_CHAT_MODEL'] = model

        if provider == 'openai':
            AUTOFIGURE_CONFIG['OPENAI_API_KEY'] = api_key
            AUTOFIGURE_CONFIG['OPENAI_BASE_URL'] = base_url
            if model:
                AUTOFIGURE_CONFIG['OPENAI_MODEL'] = model
        elif provider == 'zhipuai':
            AUTOFIGURE_CONFIG['ZHIPUAI_API_KEY'] = api_key
            AUTOFIGURE_CONFIG['ZHIPUAI_BASE_URL'] = base_url
            if model:
                AUTOFIGURE_CONFIG['ZHIPUAI_MODEL'] = model
        elif provider == 'deepseek':
            AUTOFIGURE_CONFIG['DEEPSEEK_API_KEY'] = api_key
            AUTOFIGURE_CONFIG['DEEPSEEK_BASE_URL'] = base_url
            if model:
                AUTOFIGURE_CONFIG['DEEPSEEK_MODEL'] = model
        elif provider == 'siliconflow':
            AUTOFIGURE_CONFIG['SILICONFLOW_API_KEY'] = api_key
            AUTOFIGURE_CONFIG['SILICONFLOW_BASE_URL'] = base_url
            if model:
                AUTOFIGURE_CONFIG['SILICONFLOW_MODEL'] = model
        return

    # Fallback for unknown providers: treat as OpenAI-compatible custom endpoint.
    AUTOFIGURE_CONFIG['BIANXIE_API_KEY'] = api_key
    AUTOFIGURE_CONFIG['BIANXIE_BASE_URL'] = base_url
    if model:
        AUTOFIGURE_CONFIG['BIANXIE_CHAT_MODEL'] = model
...
def create_session():
    ...
    try:
        ...
        input_content = _extract_input_content(data, config=config)
        input_type = data.get('input_type', 'text')  # text or pdf

        if not input_content:
            # Debug aid: log the raw request payload to help diagnose missing inputs
            try:
                _raw = request.get_data(as_text=True)
            except Exception:
                _raw = ''
            _log(f"[AutoFigure] DEBUG: missing input_content; raw payload: {_raw[:1000]}")
            return create_validation_error('Input content is required')
        ...
        session_data = {
            ...
            'status': 'idle',
            'config': _build_session_config(data),
        ...
        return handle_exception(e, 'Failed to create session')
...
def start_generation(session_id: str):
    ...
    try:
        if session_id not in autofigure_sessions:
            return create_not_found_error('Session')

        session = autofigure_sessions[session_id]
    except Exception as e:
        return handle_exception(e, 'Failed to process session')
        ...
        if not current_xml:
            return create_validation_error('Current XML is required')

        # Check iteration limit
        config = session['config']
        if session['current_iteration'] >= config['max_iterations']:
            return create_validation_error('Maximum iterations reached')
        ...
        try:
            ...
            if success and os.path.exists(tmp_path):
                ...
                return create_internal_error('Failed to prepare image for evaluation')
        except Exception as e:
            print(f"[AutoFigure] Error converting XML to PNG for evaluation: {e}")
            return create_internal_error('Error preparing image for evaluation')
        ...
        if not improved_xml:
             ...
             return create_internal_error('Failed to generate improved diagram. The LLM did not return valid XML. Please try again.')
        ...
        try:
            ...
            if success and os.path.exists(tmp_path):
                ...
                return create_internal_error('Failed to generate preview image for improved diagram')
        except Exception as e:
            print(f"[AutoFigure] Error converting improved XML to PNG: {e}")
            return create_internal_error('Error generating preview image for improved diagram')
        ...
        return handle_exception(e, 'Iteration failed')
...
def finalize_layout(session_id: str):
    ...
    try:
        ...
        if not final_xml:
            return create_validation_error('Final XML is required')
        ...
        try:
            ...
            if success and os.path.exists(tmp_path):
                ...
                return create_internal_error('Failed to generate preview PNG')
        except Exception as png_error:
            print(f"[AutoFigure] Error generating preview PNG: {png_error}")
            return create_internal_error('Error generating preview PNG')
        ...
        import traceback
        traceback.print_exc()
        return handle_exception(e, 'Finalization failed')
    ...
    """Start the image beautification/enhancement process using REAL implementation."""
     ...
     try:
         ...
         if not art_style or not art_style.strip():
             return create_validation_error('Art style description is required. Please describe the visual style you want.')

         if not image_gen_config['api_key']:
             return create_validation_error('Image generation API key is required')

         if not image_gen_config['model']:
             return create_validation_error('Image generation model is required')

         if not image_gen_config['base_url']:
             return create_validation_error('Image generation base URL is required')

         if mode == 'code2prompt' and not enhancement_llm_config['api_key']:
             return create_validation_error('LLM API key is required for code2prompt mode')

         if not session.get('final_xml'):
             return create_validation_error('Layout must be finalized before enhancement')
        ...
        return handle_exception(e, 'Enhancement failed')
...
def get_enhancement_status(session_id: str):
    ...
    try:
        ...
        return handle_exception(e, 'Status check failed')
...
@verify_token
    ...
    try:
        ...
        return handle_exception(e, 'Failed to get session')
...
def delete_session(session_id: str):
    ...
    try:
        ...
        return handle_exception(e, 'Failed to delete session')
...
def _run_enhancement(session_id: str, final_xml: str, mode: str, art_style: str,
    ...
    try:
        ...
        llm_provider = _normalize_provider_name(enhancement_llm_config.get('provider'), fallback='zhipuai')
        ...
        img_provider = _normalize_provider_name(image_gen_config.get('provider'), fallback='zhipuai')
        ...
        if not img_base_url:
            img_base_url = _default_base_url(img_provider)
...
def generate_image():
    ...
    try:
        ...
        provider = _normalize_provider_name(data.get('provider'), fallback='zhipuai')
        api_key = data.get('api_key', '').strip()
        model = data.get('model', '').strip()
        base_url = (data.get('base_url', '') or '').strip() or _default_base_url(provider)

        # Validate required fields - all must be provided by user
        if not prompt:
            return create_validation_error('Prompt is required')
        if not api_key:
            return create_validation_error('API key is required')
        if not model:
            return create_validation_error('Model is required')
...
Generate the image now."""
        ...
        if provider == 'gemini':
            ...
            # OpenAI-compatible path for OpenRouter/OpenAI/ZhipuAI/DeepSeek/SiliconFlow/custom compatibles.
        ...
        if not image_base64:
            return create_internal_error('No image generated')
        ...
        return create_error_response('Image generation timed out. Please try again.', status_code=504)
    except Exception as e:
        import traceback
        traceback.print_exc()
        _log(f"[AutoFigure] Image generation error: {e}")
        return handle_exception(e, 'Image generation failed')
...
def _generate_image_bianxie(prompt: str, api_key: str, model: str, base_url: str):
    ...
    # Accept provider base URL and normalize to chat completions endpoint.
...
def _generate_image_openrouter(prompt: str, api_key: str, model: str, base_url: str):
    ...
    if response.status_code != 200:
        # Some OpenAI-compatible providers do not accept OpenRouter-only modalities.
        retry_payload = dict(payload)
        retry_payload.pop('modalities', None)
        retry_headers = dict(headers)
        retry_headers.pop('HTTP-Referer', None)
        retry_headers.pop('X-Title', None)
        ...
        _log(f"[AutoFigure] OpenRouter retry(no-modalities) status: {retry_response.status_code}")
        if retry_response.status_code == 200:
            response = retry_response
        else:
            _log(f"[AutoFigure] OpenRouter API error: {response.status_code} - {response.text[:500]}")
            _log(f"[AutoFigure] OpenRouter retry error: {retry_response.status_code} - {retry_response.text[:500]}")
            raise Exception(f'OpenAI-compatible request failed: {retry_response.status_code} - {retry_response.text[:200]}')
    ...
    if images and len(images) > 0:
        ...
        if image_url:
            _log(f"[AutoFigure] OpenRouter: Image URL available but not data URL, length: {len(image_url)}")
...
def _generate_image_zhipuai(prompt: str, api_key: str, model: str, base_url: str):
    """Compatibility shim for legacy callers."""
    return _generate_image_openrouter(prompt, api_key, model, base_url)
